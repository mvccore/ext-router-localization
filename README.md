...
Default localization configured above `en-US` is allowed automaticly. Any other requested localization (e.g. path like: `/nl-NL/product-lijst` is not used to localize request and request is redirected to default localization path: `/en-US/nl-NL/product-lijst`, which could generate error 404 - not found.
```php
