...
Default localization configured above `en-US` is allowed automaticly. Any other request (e.g. path like: `/something`) is not localized 
and it's redirected to default localization path: `/en-US/something`, which could be usefull to prevent URL typo mistakes.
